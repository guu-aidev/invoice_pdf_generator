...
- **GUIアプリ付き** — CSVをファイル選択 → 対象月を指定 → 「PDF生成」ボタンで完了
...
1. `請求書ジェネレーター.exe` をダブルクリック
2. 各CSVファイルを「参照…」から選択
3. 対象月を指定（前月が初期値）
4. 「PDF生成」をクリック → 出力フォルダにPDFが並びます

入力したパスは `settings.json` に保存され、次回起動時に復元されます。
...
- PDFデザイン参考: [docs/design_reference.html](docs/design_reference.html)

</details>

## ライセンス

[MIT License](LICENSE) — 改変・商用利用OK。
